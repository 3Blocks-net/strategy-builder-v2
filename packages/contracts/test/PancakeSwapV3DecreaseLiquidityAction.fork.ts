...
// Mints a real PancakeSwap V3 position, then Decrease (100%) bundles
// decreaseLiquidity + collect — asserting the freed tokens ACTUALLY arrive in
// the vault (not merely accrued to the position) and the liquidity drops to 0.
...
forkDescribe("PancakeSwapV3DecreaseLiquidityAction (fork)", function () {
  ...
  async function deploy() {
    ...
    await curateActions(curatedRegistry, mint, decrease);
    return { ethers, owner, vault, mint, decrease, curatedRegistry };
  }
  ...
  async function fund(ethers: any, owner: any, vault: string) {
    ...
    await (await w.transfer(vault, ethers.parseEther("0.3"))).wait();
  }
  ...
  it("100% Decrease delivers the freed tokens to the vault (bundled collect ran)", async function () {
    const { ethers, owner, vault, mint, decrease } = await deploy();
    ...
    expect((await npm.positions(tokenId)).liquidity).to.be.greaterThan(0n);
    ...
    const wbnbBefore = (await wbnb.balanceOf(await vault.getAddress())) as bigint;
    ...
    expect((await npm.positions(tokenId)).liquidity).to.equal(0n);
    // Freed tokens ACTUALLY arrived in the vault (collect ran, not just accrued).
    const usdtAfter = (await usdt.balanceOf(await vault.getAddress())) as bigint;
    const wbnbAfter = (await wbnb.balanceOf(await vault.getAddress())) as bigint;
    expect(usdtAfter + wbnbAfter).to.be.greaterThan(usdtBefore + wbnbBefore);
  });
});
