...
async function main() {
  ...
  const aaveWithdrawAction = await ethers.deployContract("AaveV3WithdrawAction", [
    aaveRegistryAddr,
  ]);
  ...
  const aaveRepayAction = await ethers.deployContract("AaveV3RepayAction", [
    aaveRegistryAddr,
  ]);
  ...
  const pcsMintAction = await ethers.deployContract("PancakeSwapV3MintAction", [
    pcsRegistryAddr,
  ]);
  ...
  const pcsDecreaseAction = await ethers.deployContract("PancakeSwapV3DecreaseLiquidityAction", [
    pcsRegistryAddr,
  ]);
  const pcsDecreaseActionAddr = await pcsDecreaseAction.getAddress();
  console.log(`  PancakeSwapV3DecreaseLiquidityAction: ${pcsDecreaseActionAddr}`);

  const pcsCollectAction = await ethers.deployContract("PancakeSwapV3CollectAction", [
    pcsRegistryAddr,
  ]);
...
PancakeSwapV3DecreaseLiquidity: ${pcsDecreaseActionAddr}
PancakeSwapV3CollectAction:  ${pcsCollectActionAddr}
