...
- **Friendly params + encode-boundary mapper** (PEC-217): condition/action params are stored **friendly** in `node.data.params` (durations as `{ value, unit }`, etc.) for lossless round-trip. The frontend runs the **encode-boundary mapper** (`features/automation-editor/lib/encode-boundary.ts`, using `shared`'s `toSeconds`) just before `POST /encode` to convert friendly → **raw** and strip friendly-only fields, sending the mapped graph as `body.graph`. The encode controller prefers `body.graph` over the persisted (friendly) `editorState`; `EncodingService` runs a defensive **raw-mode guard** (`shared` `validateParams(schema, params, { mode: 'raw' })`) before building calldata (e.g. rejects `interval = 0` with HTTP 400). The generic ABI encoder is unchanged. `/step-types` (findAll) returns `paramSchema` + `abiFragment` so the editor store can do **node-init** default materialization (incl. deterministic `__time_<nodeId>` slot names) and a schema-driven **param-validation pass** over all nodes (friendly mode) merged into `validationErrors` — gating Deploy and feeding inline per-field errors (`ValidationError.fieldName`). Widget UI lives in `dynamic-form.tsx` (`x-ui-widget: duration` → `DurationField`).
