...
(in later slices) operate the vaults of **one** wallet. It reads an encrypted keystore
(password from the OS keychain), derives the owner address, and authenticates with the
backend via a server-signed **SIWE** message — then every tool is bound to that owner
(foreign-vault access is impossible). It reuses the **shared** encode-boundary so AI-built
graphs pass the same validation as the web UI.

Read/discovery tools today: `whoami`, `list_vaults`, `get_vault`, `get_portfolio`,
`list_automations`, `get_executions`, `get_positions`, `get_performance`,
`get_value_history`, `list_step_types`, `describe_step_type`. Writing/money-moving tools
(create vault, deposit/withdraw, deploy automation) follow in later slices behind a
server-enforced confirm gate.

Setup, Claude Desktop registration, and the security model: see
