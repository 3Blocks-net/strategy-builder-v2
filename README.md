...
- **PancakeSwap V3** — Swap (single-hop, mandatory slippage tolerance enforced on-chain against the pool TWAP), LP Mint (explicit price range or preset ±% width, position NFT held by the vault via `onERC721Received`), Increase / Decrease (bundles `decreaseLiquidity` + `collect`) / Collect, and **Swap-to-Range-Ratio** (on-chain, execution-time sizing: reads the live price, computes the target token0/token1 ratio for the range and swaps the over-represented token — pair it before a `Mint(full balance)`).
